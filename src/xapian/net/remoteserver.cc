      ...
      active_timeout(active_timeout_), idle_timeout(idle_timeout_)
{
    ...
    try {
	if (!dbpaths_.empty()) {
	    // Directory selection: if the server was started with database
	    // paths, open them now.  A server started with no default
	    // directory leaves db unset until the client selects one via
	    // MSG_READACCESS / MSG_WRITEACCESS.
...
RemoteServer::msg_writeaccess(const string & msg)
{
    ...
    if (p != p_end) {
	// Directory selection: the client asked to write to a specific
	// database.  Open it read-only first; db->lock() below upgrades it.
	std::vector<string> dbpaths_;
	string dbpath;
	if (!unpack_string(&p, p_end, dbpath)) {
	...
	// Server started without a default database; open the configured one.
	select_db(dbpaths, false, Xapian::DB_OPEN);
    }
