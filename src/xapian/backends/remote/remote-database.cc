...
#include <signal.h>
#include "xapian/common/safesyssocket.h" // For MSG_NOSIGNAL.
...
#include "xapian/backends/remote/remote_keylist.h"
...
RemoteDatabase::update_stats(message_type msg_code, const string & body) const
{
	...
	if (message.size() < 3) {
	    throw Xapian::NetworkError("Database was not selected", link.get_context());
	}
