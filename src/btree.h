...
class btree_node {
 ...
 public:
	...
	void value_swap(int i, btree_node *x, int j) {
		params_type::swap(&value(i), &x->value(j));
	}
	...
	void insert_value(int i, ValueType &&x);
 ...
 private:
	void value_init(int i) {
		new (&fields_.values[i]) value_type;
	}
	template <typename ValueType>
	void value_init(int i, ValueType &&x) {
		new (&fields_.values[i]) value_type(std::forward<ValueType>(x));
	}
...
class btree : public Params::key_compare {
 ...
 public:
	...
	template <typename ValueType>
	std::pair<iterator, bool> insert_unique(const key_type &key, ValueType &&value);

	// Inserts a value into the btree only if it does not already exist. The
	// boolean return value indicates whether insertion succeeded or failed.
	template <typename ValueType>
	std::pair<iterator, bool> insert_unique(ValueType &&v) {
		return insert_unique(params_type::key(v), std::forward<ValueType>(v));
	}
	...
	template <typename ValueType>
	iterator insert_unique(iterator position, ValueType &&v);
	...
	template <typename ValueType>
	iterator insert_multi(iterator position, ValueType &&v);
 ...
 private:
	...
	template <typename ValueType>
	iterator internal_insert(iterator iter, ValueType &&v);
...
////
// btree_node methods
template <typename P> template <typename ValueType>
inline void btree_node<P>::insert_value(int i, ValueType &&x) {
	assert(i <= count());
	value_init(count(), std::forward<ValueType>(x));
...
btree<P>::insert_unique(const key_type &key, ValueType &&value) {
	if (empty()) {
	...
	return std::make_pair(internal_insert(iter, std::forward<ValueType>(value)), true);
}

template <typename P> template <typename ValueType>
inline typename btree<P>::iterator
btree<P>::insert_unique(iterator position, ValueType &&v) {
	if (!empty()) {
		...
		if (position == end() || compare_keys(key, position.key())) {
			...
			if (position == begin() || compare_keys((--prev).key(), key)) {
				...
				return internal_insert(position, std::forward<ValueType>(v));
			}
			...
			if (next == end() || compare_keys(key, next.key())) {
				...
				return internal_insert(next, std::forward<ValueType>(v));
			}
		} else {
			// position.key() == key
			return position;
	...
	return insert_unique(std::forward<ValueType>(v)).first;
}
...
btree<P>::insert_multi(const key_type &key, ValueType &&value) {
	if (empty()) {
		*mutable_root() = new_leaf_root_node(1);
	}
	...
	return internal_insert(iter, std::forward<ValueType>(value));
}

template <typename P> template <typename ValueType>
typename btree<P>::iterator
btree<P>::insert_multi(iterator position, ValueType &&v) {
	if (!empty()) {
		const key_type &key = params_type::key(v);
...
template <typename P> template <typename ValueType>
inline typename btree<P>::iterator
btree<P>::internal_insert(iterator iter, ValueType &&v) {
	if (!iter.node->leaf()) {
	...
	iter.node->insert_value(iter.position, std::forward<ValueType>(v));
	return iter;
}
...
// A common base class for btree_set, map, btree_multiset and
// btree_multimap.
...
class btree_unique_container : public btree_container<Tree> {
 ...
 public:
	...
	template <typename P>
	std::pair<iterator, bool> insert(iterator position, P&& x) {
		return this->tree_.insert_unique(position, std::forward<P>(x));
	}
	...
	template <typename... Args>
	iterator try_emplace(iterator position, const key_type& k, Args&&... args) {
		auto it = find(k);
		if (it == this->end()) {
	...
	template <typename... Args>
	iterator try_emplace(iterator position, key_type&& k, Args&&... args) {
		auto it = find(k);
		if (it == this->end()) {
			return insert(position, value_type(
				std::piecewise_construct,
...
template <typename Tree>
class map_container : public btree_unique_container<Tree> {
	typedef map_container<Tree> self_type;
 ...
 public:
	// Default constructor.
	map_container(const key_compare &comp = key_compare(),
											const allocator_type &alloc = allocator_type())
			: super_type(comp, alloc) {
	}

	// Copy constructor.
	map_container(const self_type &x)
			: super_type(x) {
	}

	// Range constructor.
	template <class InputIterator>
	map_container(InputIterator b, InputIterator e,
											const key_compare &comp = key_compare(),
											const allocator_type &alloc = allocator_type())
...
class map : public map_container<
	btree<map_params<Key, Value, Compare, Alloc, TargetNodeSize> > > {

	typedef map<Key, Value, Compare, Alloc, TargetNodeSize> self_type;
	typedef map_params<
		Key, Value, Compare, Alloc, TargetNodeSize> params_type;
	typedef btree<params_type> btree_type;
	typedef map_container<btree_type> super_type;
...
inline void swap(map<K, V, C, A, N> &x,
								 map<K, V, C, A, N> &y) {
	x.swap(y);
}
